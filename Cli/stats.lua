...
local function run(msg, matches)
  if matches[1]:lower() == 'blackmask' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /blackmask ")
    return about
  end 
  ...
  if matches[1]:lower() == "stats" then
    ...
    if matches[2] == "blackmask" then -- Put everything you like :)
...
return {
  patterns = {
    ...
    "^[!/]([Ss]tatslist)$",
    "^[!/]([Ss]tats) (group) (%d+)",
    "^[!/]([Ss]tats) (blackmask)",-- Put everything you like :)
		"^[!@#$%^&*\/]([Bb]lackmask)"-- Put everything you like :)
    }, 
  run = run
}
